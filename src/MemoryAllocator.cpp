...
void MemoryAllocator::mem_construct() {
    ...
    freeMemHead->size = (uint64)HEAP_END_ADDR - (uint64)HEAP_START_ADDR - HEADER_SIZE;
    freeMemHead->alloc_verify = false;
}
...
void* MemoryAllocator::sys_mem_alloc(size_t size) {
    ...
    for (BlockHeader* curr = freeMemHead; curr != nullptr; prev = curr, curr = curr->next) {
        if (curr->size >= size) {
            if (curr->size >= size + HEADER_SIZE) {
                BlockHeader* new_free_block = (BlockHeader*)((char*)curr + HEADER_SIZE + size);
                new_free_block->size = curr->size - size - HEADER_SIZE;
                new_free_block->next = curr->next;
                new_free_block->alloc_verify = false;
...
int MemoryAllocator::sys_mem_free(const void* ptr) {
    ...
    BlockHeader* to_free = (BlockHeader*)((char*)ptr - HEADER_SIZE);
    if (to_free < HEAP_START_ADDR || to_free > HEAP_END_ADDR) return -1;
    if (!to_free->alloc_verify) return -2;

    to_free->alloc_verify = false;

    BlockHeader* prev = nullptr;
    BlockHeader* curr = freeMemHead;

    for (; curr != nullptr; prev = curr, curr = curr->next) {
        if (to_free < curr) break;
    }
...
void MemoryAllocator::merge(BlockHeader* blk1, BlockHeader* blk2) {
    if (!blk1 || !blk2) return;

    if ((char*)blk1 + HEADER_SIZE + blk1->size == (char*)blk2) {
        blk1->size += HEADER_SIZE + blk2->size;
        blk1->next = blk2->next;
    }
...
//         printString("[");
//         printInt(reinterpret_cast<uint64>(current));
//         printString(" | Size: ");
//
...
void MemoryAllocator::deallocateEverything() {
    freeMemHead = (BlockHeader*)(HEAP_START_ADDR);
    freeMemHead->next = nullptr;
    freeMemHead->size = (uint64)HEAP_END_ADDR - (uint64)HEAP_START_ADDR - HEADER_SIZE;
    freeMemHead->alloc_verify = false;
